<<<<<<< HEAD:Lab4/Lab4_tidy_data_Incompleted.Rmd
---
...
Import the curated dataset we generated last week, verify that all column names are in snake case, and any measurements include units. 
```{r}
scfa_data <- read_delim(file = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab3/curated_data/scfa_indv_qc.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
```
...
scfa_wkly %>%
  ...
  filter(study_week == "week1" | study_week == "week3") %>% #keep only weeks of interest
  spread(key = study_week, value = butyrate_mean_mmol_kg) #widen data frame
```
...
butyrate_wide <- scfa_wkly %>%
  ...
  filter(study_week == "week1" | study_week == "week3") %>% #drop extra weeks
  spread(key = study_week, value = butyrate_mean_mmol_kg) %>% #convert to wide format
  na.omit(week1, week3) %>% #drop NAs from select columns
  mutate(delta_butyrate = week3 - week1) #calculate difference 
...
acetate_wide <- scfa_wkly %>%
  ...
  spread(key = study_week, value = acetate_mean_mmol_kg) %>% 
  na.omit(week1, week3) %>% 
...
propionate_wide <- scfa_wkly %>%
  ...
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(key = study_week, value = propionate_mean_mmol_kg) %>% 
  na.omit(week1, week3) %>% 
...
```{r}
# write code here
butyrate_type <- butyrate_wide%>% 
  group_by(supplement_consumed) %>% 
  summarize(mean_butyrate = mean(delta_butyrate, na.rm = TRUE))
```

Repeat this process for each SCFA. How would you plot your results? 
```{r}
# copy+paste and edit code chunk for acetate
acetate_type <- acetate_wide%>% 
  group_by(supplement_consumed) %>% 
  summarize(mean_acetate = mean(delta_acetate, na.rm = TRUE))
```

```{r}
# copy+paste and edit code chunk for propionate
propionate_type <- propionate_wide%>% 
  group_by(supplement_consumed) %>% 
  summarize(mean_propionate = mean(delta_propionate, na.rm = TRUE))
```

```{r}
# copy+paste and edit code chunks for plots
plot_but_type <- ggplot(data = butyrate_type, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_butyrate)) +
    geom_point() +
    labs(title = "The average difference of fecal butyrate by supplement type",
         x = "Supplement type",
         y = "Fecal butyrate (mmol/kg)") 
plot_ace_type <- ggplot(data = acetate_type, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_acetate)) +
    geom_point() +
    labs(title = "The average difference of fecal acetate by supplement type",
         x = "Supplement type",
         y = "Fecal acetate (mmol/kg)") 
plot_pro_type <- ggplot(data = propionate_type, 
       mapping = aes(x = supplement_consumed, 
                     y = mean_propionate)) +
    geom_point() +
    labs(title = "The average difference of fecal propionate by supplement type",
         x = "Supplement type",
...
total_scfas <- inner_join(acetate_wide, butyrate_wide,
                ...
                inner_join(propionate_wide,
                          by = c("participant_id", "semester", "supplement_consumed")) %>%
  mutate(total_scfa_wk1 = ace_wk1 + but_wk1 + pro_wk1,
         total_scfa_wk3 = ace_wk3 + but_wk3 + pro_wk3,
...
plot_scfa <- total_scfas %>%
  ...
  ggplot(aes(x = supplement_consumed, y = delta_total_scfa)) +
...
# copy+paste and edit code here
wkly_means <- scfa_long %>%
  group_by(study_week, participant_id, supplement_consumed,scfa, semester) %>% 
  summarize(mean_scfa = mean(concentration_mmol_kg))
wkly_means
...
Save the wide data frame to the curated data directory.
```{r}
write_delim(total_scfas, path = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/curated_data/total_scfas_wide.txt", delim = "\t")
```

Save the other data frames and plots generated today. Be sure they are placed in the appropriate directories (folders):
```{r}
# for data frames:
write_delim(scfa_long, path = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/curated_data/scfa_long.txt", delim = "\t")
write_delim(wkly_means, path = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/curated_data/wkly_means.txt", delim = "\t")
bristol_numeric
# for plots:
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/figures/plot_scfa.pdf",
          plot = plot_scfa)
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/figures/p1.pdf",
          plot = p1)
...
```{r}
# import pH data
pH_data <- read_delim(file = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/raw_data/pH_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)

# filter to remove bad measurements based on outside list
pH_bad <- read_delim(file = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/raw_data/pH_bad.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>% #import list of samples with date discrepencies 
  rename_all(tolower)
...
```{r}
# import BSS
BSS_data <- read_delim(file = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/raw_data/BSS_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
...
pH_BSS_wkly <- full_join(pH_qc, BSS_qc,
  ...
  mutate(wkly_mean_bristol_numeric = round(wkly_mean_bristol_numeric, digits = 0),
         wkly_mean_ph = round(wkly_mean_ph, digits = 0)) #round avgs to reflect sig figs
```
...
Now that the data are ready, save it as a CSV file in the curated_data directory.
```{r}
write_delim(pH_BSS_wkly, path = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab4/curated_data/ph_BSS_wkly.csv", delim = "\t")
```


-----
>>>>>>> ebf15512e65dc4d0438e2b2d5428d419c02e8777:Lab4/Lab4_tidy_data_completed.Rmd
end