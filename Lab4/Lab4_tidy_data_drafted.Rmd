...
scfa_data <- read_delim(file = "Lab3/curated_data/scfa_indv_qc.txt",
                        ...
                        na = c("", "NA")) 
scfa_wkly_qc2 <- read_delim(file="Lab3/curated_data/scfa_wkly_qc2.txt",  delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
...
scfa_data %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(butyrate_mmol_kg, na.rm = TRUE), mean_acetate = mean (acetate_mmol_kg, na.rm = TRUE), mean_propionate = mean(propionate_mmol_kg, na.rm = TRUE))
...
acetate_wide <- scfa_wkly_qc2 %>%
  select(participant_id, semester, supplement_consumed, study_week, acetate_mean) %>% 
...
propionate_wide <- scfa_wkly_qc2 %>%
  select(participant_id, semester, supplement_consumed, study_week, propionate_mean) %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
...
```{r}
# write code here
mean_but<- butyrate_wide %>%
group_by(supplement_consumed) %>%
summarise(mean_difference_but = mean(delta_butyrate))
```

Repeat this process for each SCFA. How would you plot your results? 
```{r}
# copy+paste and edit code chunk for acetate
mean_act <- acetate_wide %>%
group_by(supplement_consumed) %>%
summarise(mean_difference_ace = mean(delta_acetate))

```

```{r}
# copy+paste and edit code chunk for propionate
mean_prop  <- propionate_wide %>%
group_by(supplement_consumed) %>%
summarise(mean_difference_prop = mean(delta_propionate))
```
...
plot_f_but <- mean_but %>%
   ggplot(aes(x = supplement_consumed, 
             y = mean_difference_but)) +
    geom_point(color = "black") +
    labs(title = "mean difference of butryate",
         x = "suplements type",
         y = "mean difference ") +
...
plot_f_act <- mean_act  %>%
   ggplot(aes(x = supplement_consumed, 
             y = mean_difference_ace)) +
    geom_point(color = "red") +
    labs(title = "mean difference of acetate",
         x = "suplements type",
         y = "mean difference ") +
    theme(axis.text.x = element_blank(),
          legend.position = "none")

plot_f_prop <- mean_prop %>%
   ggplot(aes(x = supplement_consumed, 
             y = mean_difference_prop)) +
    geom_point(color = "green") +
    labs(title = "mean difference of propionate",
         x = "suplements type",
         y = "mean difference ") +
    theme(axis.text.x = element_blank(),
          legend.position = "none")
plot_grid(plot_f_but, plot_f_act, plot_f_prop,
          nrow = 1, ncol = 3)
plot_grid
```
...
# write code here
total_scfas <- join3 %>%
  mutate(totl_wk1 = ace_wk1 + but_wk1 + pro_wk1, totl_wk3 = ace_wk3 + but_wk3 + pro_wk3) %>% 
  na.omit(delta_acetate, delta_butyrate, delta_propionate) %>%
  mutate(totldif = delta_acetate+ delta_butyrate + delta_propionate) 
...
```{r}
# write code here
plot_tscfa <- total_scfas %>%
ggplot(aes(x = supplement_consumed, 
             y = totldif, 
              supplement_consumed, semester)) +
  geom_point(aes(color = "totldif")) +
...
scfa_long <- inner_join(ace_long, pro_long, 
                    by = c("participant_id", "semester", "supplement_consumed", "totl_wk1", "totl_wk3", "totldif", "study_week"))  %>%
inner_join(but_long,
           by = c("participant_id", "semester", "supplement_consumed", "totl_wk1", "totl_wk3", "totldif", "study_week"))
...
group_by(participant_id,study_week, semester, supplement_consumed, scfa) %>%
  summarize(meanwkly= mean(concentration_mmol_kg))
```
...
p1 <- wkly_means %>%
  ...
  ggplot(aes(x = semester, y = meanwkly, scfa)) +
...
```{r}
# for data frames:
write_delim(wkly_means, path = "Lab4/curated_data/wkly_means_long.txt", delim = "\t") 
write_delim(scfa_long, path = "Lab4/curated_data/scfa_long.txt", delim = "\t") 

# for plots:
save_plot(filename = "Lab4/figures/p1_plot.pdf",
          plot = p1,
          nrow = 3, ncol = 4, 
          base_aspect_ratio = 1.1)
save_plot(filename = "Lab4/figures/plot_tscfa.pdf", 
          plot=plot_tscfa,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
```
...
pH_qc <- pH_data %>%
  ...
  summarise(wkly_avg_ph = mean(ph)) #calculate weekly average
dim(pH_qc)
```
...
BSS_qc <- BSS_data %>%
  ...
  group_by(participant_id, study_week) %>%
  summarise(wkly_avg_bss = mean(bristol_numeric)) #calculate weekly average 
dim(BSS_qc)
```
...
pH_BSS_wkly <- full_join(pH_qc, BSS_qc,
  ...
  mutate(wkly_avg_bss= round(wkly_avg_bss , digits=2), wkly_avg_ph = round(wkly_avg_ph, digits = 2)) #round avgs to reflect sig figs
```
