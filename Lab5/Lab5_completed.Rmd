...
```{r include=FALSE}
scfa_long <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/curated_data/SCFA_wkly_long.txt", 
...
scfa_wide <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/curated_data/SCFA_wkly_wide.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>%
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
#0.2367 is a large p-value, which indicates the means of the two groups are not different.
```
...
```{r}
# breath methane
breath_long <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/curated_data/breath_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>% 
   mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
methane_long <- breath_long %>%
  select(-starts_with("h2"), -starts_with("co2"),
         -starts_with("n"), 
         -ends_with("median"),
         -"comments") %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" | supplement_consumed == "LOODAT",
         quantity_compliant == "yes") %>%
 rename(methane_mean = ch4_mean) %>% 
 na.omit(methane_mean)


plot_methane <- methane_long %>%
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_methane.pdf",
          plot = plot_methane)  
```
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_acetate.pdf",
          plot = plot_acetate)
```
...
plot_propionate <- propionate_long %>%
  filter(study_week == "week3" | study_week == "week1") %>% 
  ggplot(aes(x = study_week, 
             y = propionate_mean)) + 
  geom_violin() 
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_propionate.pdf",
...
```{r}
# pH
ph_long <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/curated_data/pH_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>% 
   mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_ph.pdf",
          plot = plot_ph)  
...
# normal distribution 
methane_df <- methane_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS",
         quantity_compliant == "yes",
         study_week == "week3" | study_week == "week1")

shapiro.test(methane_df$methane_mean) #call column of df with values = vector 

ggplot(methane_df, aes(x=methane_mean)) +
  geom_histogram() #histograms show the number of data points (count) at each value

qqnorm(methane_df$methane_mean); qqline(methane_df$methane_mean)

# equal variances
week1_methane_grp <- methane_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS",
         quantity_compliant == "yes",
         study_week == "week1") 

week3_methane_grp <- methane_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" ,
         quantity_compliant == "yes",
         study_week == "week3") 

var.test(x = week1_methane_grp$methane_mean, 
         y = week3_methane_grp$methane_mean, 
         alternative = "two.sided")
```
...
# normal distribution 
acetate_df <- acetate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" ,
         quantity_compliant == "yes",
         study_week == "week3" | study_week == "week1")

shapiro.test(acetate_df$acetate_mean) #call column of df with values = vector 

ggplot(acetate_df, aes(x=acetate_mean)) +
  geom_histogram() #histograms show the number of data points (count) at each value

qqnorm(acetate_df$acetate_mean); qqline(acetate_df$acetate_mean)

# equal variances
week1_acetate_grp <- acetate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" ,
         quantity_compliant == "yes",
         study_week == "week1") 

week3_acetate_grp <- acetate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS",
         quantity_compliant == "yes",
         study_week == "week3") 

var.test(x = week1_acetate_grp$acetate_mean, 
         y = week3_acetate_grp$acetate_mean, 
         alternative = "two.sided")
```
...
# normal distribution 
propionate_df <- propionate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" ,
         quantity_compliant == "yes",
         study_week == "week3" | study_week == "week1")

shapiro.test(propionate_df$propionate_mean) #call column of df with values = vector 

ggplot(propionate_df, aes(x=propionate_mean)) +
  geom_histogram() #histograms show the number of data points (count) at each value

qqnorm(propionate_df$propionate_mean); qqline(propionate_df$propionate_mean)

# equal variances
week1_propionate_grp <- propionate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS" ,
         quantity_compliant == "yes",
         study_week == "week1") 

week3_propionate_grp <- propionate_long %>%
  filter(semester != "Winter2015",
         supplement_consumed == "BRMPS",
         quantity_compliant == "yes",
         study_week == "week3") 

var.test(x = week1_propionate_grp$propionate_mean, 
         y = week3_propionate_grp$propionate_mean, 
         alternative = "two.sided")
```
...
shapiro.test(ph_df$ph_mean) #call column of df with values = vector 
...
```{r}
# breath methane
methane_df_2 <- inner_join(x = week1_methane_grp, y = week3_methane_grp,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
  rename(methane_mean_wk1 = methane_mean.x,
         methane_mean_wk3 = methane_mean.y) %>%
  select(-starts_with("study_week"))

# code to run paired t-test 
t.test(x = methane_df_2$methane_mean_wk1, y = methane_df_2$methane_mean_wk3, 
       alternative = "less", paired = TRUE) 

#The p-value is 0.0086 which is low. The pattern we observed in the figure is likely a real trend. We can say with confidence that the methane concentrations increases between weeks 1 and 3.
```

```{r}
# acetate
acetate_df_2 <- inner_join(x = week1_acetate_grp, y = week3_acetate_grp,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
# code to run paired t-test 
t.test(x = acetate_df_2$acetate_mean_wk1, y = acetate_df_2$acetate_mean_wk3, 
       alternative = "less", paired = TRUE) 

#The p-value is 0.0073 which is low. The pattern we observed in the figure is likely a real trend. We can say with confidence that the acetate concentrations increases between weeks 1 and 3.
```

```{r}
# propionate
propionate_df_2 <- inner_join(x = week1_propionate_grp, y = week3_propionate_grp,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
# code to run paired t-test 
t.test(x = propionate_df_2$propionate_mean_wk1, y = propionate_df_2$propionate_mean_wk3, 
       alternative = "less", paired = TRUE) 

#The p-value is 0.3 which is high. The pattern we observed in the figure is not likely a real trend. We cannot say with confidence that the propionate concentrations increases between weeks 1 and 3.
```

```{r}
# pH
ph_df_2 <- inner_join(x = week1_ph_grp, y = week3_ph_grp,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
# code to run paired t-test 
t.test(x = ph_df_2$ph_mean_wk1, y = ph_df_2$ph_mean_wk3, 
       alternative = "greater", paired = TRUE) 

#The p-value is 0.0029 which is high. The pattern we observed in the figure is likely a real trend. We can say with confidence that the ph decreases between weeks 1 and 3.
```
...
plot_ace
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_ace_anova.pdf",
          plot = plot_ace)


y1 <- scfa_wide %>%
  filter(supplement_consumed == "Accessible") 
shapiro.test(y1$delta_acetate) 

y2 <- scfa_wide %>%
  filter(supplement_consumed == "Arabino") 
shapiro.test(y2$delta_acetate) 

y3 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS+Accessible")
shapiro.test(y3$delta_acetate) 

y4 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize") 
shapiro.test(y4$delta_acetate) 

y5 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize+BRMPS") 
shapiro.test(y5$delta_acetate) 

y6 <- scfa_wide %>%
  filter(supplement_consumed == "Inulin") 
shapiro.test(y6$delta_acetate) 

y7 <- scfa_wide %>%
  filter(supplement_consumed == "LOODAT") 
shapiro.test(y7$delta_acetate)

y8 <- scfa_wide %>%
  filter(supplement_consumed == "none") 
shapiro.test(y8$delta_acetate)

y9 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium") 
shapiro.test(y9$delta_acetate)

y10 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium+BRMPS") 
shapiro.test(y10$delta_acetate)

y11 <- scfa_wide %>%
  filter(supplement_consumed == "transition_HiMaize") 
shapiro.test(y11$delta_acetate)

y12 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(y12$delta_acetate)
```
...
plot_pro
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab5/figures/plot_pro_anova.pdf",
          plot = plot_pro)

z1 <- scfa_wide %>%
  filter(supplement_consumed == "Accessible") 
shapiro.test(z1$delta_propionate) 

#z2 <- scfa_wide %>%
 # filter(supplement_consumed == "Arabino") 
#shapiro.test(z2$delta_propionate) 

z3 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS+Accessible")
shapiro.test(z3$delta_propionate) 

z4 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize") 
shapiro.test(z4$delta_propionate) 

#z5 <- scfa_wide %>%
  #filter(supplement_consumed == "HiMaize+BRMPS") 
#shapiro.test(z5$delta_propionate) 

z6 <- scfa_wide %>%
  filter(supplement_consumed == "Inulin") 
shapiro.test(z6$delta_propionate) 

z7 <- scfa_wide %>%
  filter(supplement_consumed == "LOODAT") 
shapiro.test(z7$delta_propionate)

z8 <- scfa_wide %>%
  filter(supplement_consumed == "none") 
shapiro.test(z8$delta_propionate)

z9 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium") 
shapiro.test(z9$delta_propionate)

z10 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium+BRMPS") 
shapiro.test(z10$delta_propionate)

z11 <- scfa_wide %>%
  filter(supplement_consumed == "transition_HiMaize") 
shapiro.test(z11$delta_propionate)

z12 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(z12$delta_propionate)
```
