...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab8/figures/plot_genus.pdf",
          plot = plot_genus)
```
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab8/figures/plot_order.pdf",
          plot = plot_order)
```
...
```{r}
ordplot1_j <- plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "semester", 
                     shape = "study_week")
ordplot2_j <- ordplot1_j +
  geom_point(size=3) + #increase point size
  scale_color_manual(name = "Semester",
                     values = c("coral3", 
                                "skyblue3", 
  ...
  scale_shape_manual(name = "Study week",
                     ...
                     labels = c("week 1", "week 3")) #change default shapes 
print(ordplot2_j)
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab8/figures/ordplot2_j.pdf",
          plot = ordplot2_j)
```
...
# run test
adn_res_jaccard <- adonis(formula = dat_jaccard ~ semester * study_week, 
                  data = sampledf_jaccard)

# view results 
print(adn_res_jaccard)

#In our results above, the p-values of 0.001 indicate that the groupings of samples by semester or by study week is statistically significant. The R value of 0.028 and 0.0077 are fairly close to 0, indicating that the grouping of samples based by semester or study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test. 
```
