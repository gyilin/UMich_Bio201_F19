...
physq_long_genus <- physq_sub %>%
  ...
  select(-sample_number, -quantity_compliant, -use_data, 
         -ends_with("_mm"), -ends_with("_mmol_kg"), 
         -starts_with("bristol"), -ph,-phylum,-class,-order,-family) %>% #drop columns not using
  filter(abundance > 0.001) %>% #remove low abund taxa
  arrange(genus) #sort alphabetically by taxa

#plot
genus_graph <- ggplot(data = physq_long_genus, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
  ...
  save_plot(filename = "Lab8/figures/genus.pdf",
          plot = genus_graph,
          nrow = 1, ncol = 8, 
...
#plot
order_tax_graph <- ggplot(data = physq_long_order, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
  ...
  facet_grid(~semester) + #panel for each semester
  xlab(NULL) + 
  theme(axis.text.x = element_text(angle = 79,
                                   vjust = 0.5))+#clean up x-axis labels 
  ylab("Relative Abundance (Order > 0.1%)")
save_plot(filename = "Lab8/figures/order.pdf",
          plot = order_tax_graph,
          nrow = 1, ncol = 8, 
          base_aspect_ratio = 1.1)
```
...
physq_j
#stress value:
#Run 0 stress 0.2330285 Run 1 stress 0.2597528 Run 2 stress 0.2518498 Run 3 stress 0.2583904 Run 4 stress 0.2497398 Run 5 stress 0.2418675 Run 6 stress 0.2640926 Run 7 stress 0.242729 Run 8 stress 0.2376124 Run 9 stress 0.2379673 Run 10 stress 0.2601498 Run 11 stress 0.2526582 Run 12 stress 0.2447506 Run 13 stress 0.2527811 Run 14 stress 0.2334993, Run 15 stress 0.2348453 Run 16 stress 0.2444402 Run 17 stress 0.4190191 Run 18 stress 0.4190068 Run 19 stress 0.2304688 Run 20 stress 0.2478321 Run 21 stress 0.2429822 Run 22 stress 0.2459456 Run 23 stress 0.2455483 Run 24 stress 0.2476332 Run 25 stress 0.2480926 Run 26 stress 0.2479847 Run 27 stress 0.2615034 Run 28 stress 0.2346342 Run 29 stress 0.2377953 Run 30 stress 0.246468 Run 31 stress 0.2389541 Run 32 stress 0.24861 Run 33 stress 0.2529081 Run 34 stress 0.2373592 Run 35 stress 0.2560642 Run 36 stress 0.24435 Run 37 stress 0.2491119 Run 38 stress 0.2420599 Run 39 stress 0.2429601 Run 40 stress 0.2336562 Run 41 stress 0.2501396 Run 42 stress 0.2533679 Run 43 stress 0.248062 Run 44 stress 0.2324274 Run 45 stress 0.2432075 Run 46 stress 0.2527952 Run 47 stress 0.2434924 Run 48 stress 0.241073 Run 49 stress 0.2412904 Run 50 stress 0.2447381 
```
...
print(ordplot2)
save_plot(filename = "Lab8/figures/Bray Curtis.pdf",
          plot = ordplot2,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
```
...
ordplot3 <- plot_ordination(physeq = physq_sub, 
  ...
  scale_shape_manual(name = "Study week",
                     ...
                     labels = c("week 1", "week 3")) #change default shapes 
print(ordplot3)
save_plot(filename = "Lab8/figures/Jaccard.pdf",
          plot = ordplot3,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
```
...
sampledf_jaccard <- physq_sub %>% 
  sample_data(.) %>% #extract sample data from phyloseq object 
  as(., "data.frame") #convert to data frame for adonis()

# run test
adn_res_j <- adonis(formula = dat_jaccard ~ semester * study_week, 
                  data = sampledf_jaccard)

# view results 
print(adn_res_j)
#In our results above, the p-values of 0.001 indicate that the groupings of samples by semester or by study week is statistically significant. The R value of 0.02871 and 0.00774. both are fairly close to 0, indicating that the grouping of samples based by semester or study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test. 
```
