...
sv_table <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/raw_data/seq_variants_wide.csv",
...
```{r}
taxa <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/raw_data/seqvar_classifications.csv",
                            delim = ",", escape_double = FALSE, 
                            trim_ws = TRUE, na=c("NA"),
                            col_types = list()) %>%
...
```{r}
all_data_indv <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/raw_data/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
...
### Calculate richness
```{r}
samp_data <- read_delim("C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/raw_data/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
                            col_types = cols(Bristol_numeric = col_number(),
                                             Bristol_score = col_character(), 
...
write_delim(richness_df, 
            path="C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/curated_data/richness_df.txt",
...
richness_wk1 <- richness_df %>%
  filter(study_week == "week1", 
         supplement_consumed == "BRMPS") 
shapiro.test(richness_wk1$avg_richness) 
ggplot(richness_wk1, aes(x = avg_richness)) + geom_histogram()

richness_wk3 <- richness_df %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(richness_wk3$avg_richness) 
ggplot(richness_wk3, aes(x = avg_richness)) + geom_histogram()

# join these data frames back together
richness_df_t_test <- inner_join(x = richness_wk1, y = richness_wk3,
                    by = c("participant_id", "semester", "supplement_consumed")) %>%
  rename(avg_richness_wk1 = avg_richness.x,
         avg_richness_wk3 = avg_richness.y) %>% 
  select(-"study_week.x", -"study_week.y")

# write code for paired t-test here 
t.test(x = richness_df_t_test$avg_richness_wk1, y = richness_df_t_test$avg_richness_wk3, 
...
Answer: The p-value is 6.977e-07 which is low. We can say with confidence that the richness changed from week 1 to week 3 in individuals who consumped BRMPS.
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/figures/rich2.pdf",
...
write_delim(simpson_df, 
            path="C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/curated_data/simpson_df.txt",
            delim = "\t", quote = FALSE, col_names = TRUE)
```
...
simpson_wk1 <- simpson_df %>%
  filter(study_week == "week1", 
         supplement_consumed == "BRMPS") 
shapiro.test(simpson_wk1$avg_richness) 
ggplot(simpson_wk1, aes(x = avg_richness)) + geom_histogram()

simpson_wk3 <- simpson_df %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 
shapiro.test(simpson_wk3$avg_richness) 
ggplot(simpson_wk3, aes(x = avg_richness)) + geom_histogram()

# join these data frames back together
simpson_df_t_test <- inner_join(x = simpson_wk1, y = simpson_wk3,
                    by = c("participant_id", "semester", "supplement_consumed")) %>%
  rename(avg_richness_wk1 = avg_richness.x,
         avg_richness_wk3 = avg_richness.y) %>% 
  select(-"study_week.x", -"study_week.y")
...
# write code for paired t-test here 
t.test(x = simpson_df_t_test$avg_richness_wk1, y = simpson_df_t_test$avg_richness_wk3, 
       alternative = "two.sided", paired = TRUE) 
# p-value = 0.08479
# Conclusions: The p-value is 0.08479 which is low. We can say with confidence that the richness changed from week 1 to week 3 in individuals who consumped BRMPS.
```
...
save_plot(filename = "C:/Users/tina5/OneDrive/桌面/UMich_Bio201_F19/Lab7/figures/simp2.pdf",
