...
nrow(scfa_indv) # 4078 rows in the data frame
ncol(scfa_indv) #19 is the number of columns
dim(scfa_indv) #4078 and 19 is the dimension of the data frame
colnames(scfa_indv) # the different column names in the data 
rownames(scfa_indv) # the numeric row names in the data frame
glimpse(scfa_indv) # gives you a glimpse of the data frame including information on both the row and the column #
str(scfa_indv) # gives you the idea about whether the data is numeric or character in the data frame about each column
```
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(sample_wt_g > 0.10) %>% 
  filter(notes != "frozen >24hrs") %>% 
  select(participant_id, supplement_consumed, butyrate_mM)
```
...
* Rename columns as described in sections above ( case, units)
...
mutate(butyrate_mmol_kg = round((butyrate_mM*0.002/ sample_kg), digits = 2)) %>%
mutate(propionate_mmol_kg = round((propionate_mM*0.002/sample_kg), digits = 2)) %>%
mutate(total_scfa_mmol_kg = round((butyrate_mmol_kg + propionate_mmol_kg + acetate_mmol_kg), digits = 2)) 
...
mutate(total_mean = (butyrate_mean + propionate_mean + acetate_mean ) /3) %>%
mutate(total_median = (butyrate_median + propionate_median + acetate_median ) /3) 
```

Export this curated data frame:
```{r}
write_delim(scfa_wkly_qc2, path = "curated_data/scfa_wkly_qc2.txt", delim = "\t")
```
