...
```{r}
scfa_indv <- rename(scfa_indv,acetate_mM = ace, butyrate_mM = but,propionate_mM = pro)
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(notes != "frozen >24hrs") %>% 
  filter(sample_wt_g > 0.10) %>% 
  select(participant_id, sample_wt_g, butyrate_mM)
```
...
scfa_indv_qc <- scfa_indv %>% 
  ...
  select(-ht_in, -wt_lbs, -sample_wt_g, -acetate_mmol_kg, -butyrate_mmol_kg, -propionate_mmol_kg)
...
scfa_wkly_qc <- read_excel("raw_data/SCFA_data.xlsx",
  ...
  mutate(total_SCFA_median_mmol_kg = acetate_median_mmol_kg + butyrate_median_mmol_kg + propionate_median_mmol_kg,
         total_SCFA_mean_mmol_kg = acetate_mean_mmol_kg + butyrate_mean_mmol_kg + propionate_mean_mmol_kg)
