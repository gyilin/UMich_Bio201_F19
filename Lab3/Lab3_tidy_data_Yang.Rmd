...
knitr::opts_knit$set(root.dir = "~/Desktop/UMich_Bio201_F19/Lab3/")
```
...
dim() # tells you dimensions (number of columns x number of rows)
colnames() # Column Label Names
rownames() # Row Label Names
glimpse() # Shows some initial datapoints with measured variables and participant #
str() # Shows data types of measured variables 
```
...
f18_but_rps <- scfa_indv %>%
  filter(sample_wt_g > 0.1 & sample_wt_g <= 1) %>%
  filter(notes != "frozen >24hrs") %>%
  filter(semester == "Fall2018") %>%
  filter(study_week == "week1" | study_week == "week3") %>%
  filter(supplement_consumed == "BRMPS") %>%
  select(participant_id, sample_number, sample_id, butyrate_mM) 
f18_but_rps
```
