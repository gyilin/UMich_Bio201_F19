...
scfa_indv <- rename_all(tolower) %>%
ht_m = round((ht_in / 35.274), digits= 0 ) %>%
wt_kg = round((wt_lbs / 2.205 ), digits = 0) %>%
filter(sample_wt_g > 0.10) %>% 
filter(notes != "frozen >24hrs") %>% 
mutate(butyrate_mmol_kg = (butyrate_mM*0.002)/sample_kg, 
       propionate_mmol_kg = (propionate_mM*0.002)/sample_kg,
       acetate_mmol_kg = (acetate_mM*0.002)/sample_kg,
        total_SCFA = butyrate_mmol_kg + propionate_mM_kg + acetate_mmol_kg) %>% 
 round(butyrate_mmol_kg, propionate_mmol_kg, acetate_mmol_kg, total_SCFA, digits= 2 ) 
...
```{r}
scfa_wkly_qc <- read_excel("SCFA_wkly",
                        sheet = "SCFA_indv", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA", "-----")) 
