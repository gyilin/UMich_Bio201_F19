...
scfa_qc <- read_delim(file = "curated_data/scfa_wkly_qc.txt", 
                        ...
                        col_types = list())%>%
# drop median columns
select(-Acetate_median, -Butyrate_median, -Propionate_median, -total_SCFA_median)%>%
...
scfa_qc <- rename(scfa_qc, total_scfa_mean_mmol_kg = total_scfa_mean)
# There is already a total_scfa_mean, so I just change the name of it.
```
...
> ** enter answer here ** 
These two plots share the same x-axis and the same y-axis. Also, colors are also different.
...
Export plot 3.6 
```{r}
save_plot(filename = "figures/facet_plot3_6.pdf",
          plot = plot3_6,
          nrow = 1, ncol = 2, 
          base_aspect_ratio = 1.1)
ggplot2::ggsave(filename = "facet_plot3_6.tiff",
                path = "figures/",
                device = "tiff",
                plot = plot3_6,
                dpi = "retina")
