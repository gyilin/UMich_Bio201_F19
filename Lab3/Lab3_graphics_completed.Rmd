...
scfa_qc <- read_delim(file = "curated_data/scfa_wkly_qc2.txt", 
...
rename(acetate_mean_mmol_kg = acetate_mean, butyrate_mean_mmol_kg = butyrate_mean, propionate_mean_mmol_kg = propionate_mean) %>%

# calculate total SCFA
mutate(total_mean_mmol_kg = acetate_mean_mmol_kg + butyrate_mean_mmol_kg + propionate_mean_mmol_kg ) 
...
```{r}
ggplot(data = scfa_qc_2, #call dataset
       mapping = aes(x = participant_id, 
...
```{r}
 plot_ace <-ggplot(data = scfa_qc_2, 
       mapping = aes(x = participant_id, 
...
```{r}
plot_but <- ggplot(data = scfa_qc_2, 
       mapping = aes(x = participant_id, 
                     y = butyrate_mean_mmol_kg, 
                     color = study_week)) +
    geom_point(color = "bisque1") +
    labs(title = "Mean butyrate concentrations of individuals",
         x = "Individuals",
         y = "Fecal butyrate (mmol/kg)")+    
  theme(axis.text.x = element_blank())
...
```{r}
plot_pro <- ggplot(data = scfa_qc_2, 
       mapping = aes(x = participant_id, 
...
```{r}
plot_tot <- ggplot(data = scfa_qc_2, 
       mapping = aes(x = participant_id, 
                     y = total_mean_mmol_kg, 
                     color = study_week)) +
    geom_point(color = "sienna3") +
    labs(title = "Mean total concentrations of individuals",
         x = "Individuals",
         y = "Fecal total scfa (mmol/kg)")+
      theme(axis.text.x = element_blank())
plot_tot
```
...
Complete any additional customizations to make the figure easy to read. Compare the facetted figure with the version you generated above where each panel was created manually. What differences in the layout of the figure do you notice? Write your answer in the bold block below: the same variables are merged here. For example, the study week is clearly defined by the color and the same varible of total mean fecal sample of each participant are represented as dots. This variable is shared among two graphs, which make the general picture looks clearer and cleaner than my own cowplots.
...
#filter(supplement_consumed == "BRMPS" | supplement_consumed == "Psyllium" | supplement_consumed == "Accessible" | supplement_consumed == "HiMaize" | supplement_consumed == "Arabino" | supplement_consumed == "insulin" | supplement_consumed == "LOODAT" | supplement_consumed == "transition_HiMaize" | supplement_consumed == "HiMaize+BRMPS"| supplement_consumed == "BRMPS+Accessible" | supplement_consumed == "BRMPS+Psyllium") 
  
ggplot(aes(x = semester, 
             y = butyrate_mean_mmol_kg, 
             study_week, supplement_consumed, semester)) +
  geom_point(aes(color = "butyrate_mean_mmol_kg")) +
  facet_grid(~supplement_consumed, scale = "free") +
  labs(x = "semster",
       y = "Mean butyrate concentrations of individuals (mmol/kg)") +
...
Export plot 3.6 
```{r}
save_plot(filename = "figures/facet_plot1.pdf",
          plot = plot3_6,
          nrow = 1, ncol = 12, 
          base_aspect_ratio = 1.1)
```
