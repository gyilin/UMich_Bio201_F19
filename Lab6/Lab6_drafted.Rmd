...
```{r}
# hypothesis: 
#It is likely that the hydrogen breath levels may differ between participants consuming fiber supplements. 
# plot
breath_wkly %>%
  ggplot(aes(x = supplement_consumed, 
             y = h2_mean, 
...
shapiro.test(s1_acc$h2_mean)
ggplot(s1_acc, aes(x = h2_mean)) + geom_histogram()+
labs(x = "h2_mean_Accesbile",
      y = "Accessible") 
#p value 0.0937

s1_ara <- breath_wkly %>%
filter (supplement_consumed ==  "Arabino") 
shapiro.test(s1_ara$h2_mean)
ggplot(s1_ara, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_Arabino",
      y = "count") 
# p value 0.1695

s1_bana <- breath_wkly %>%
filter (supplement_consumed ==  "Banana") 
shapiro.test(s1_bana$h2_mean)
ggplot(s1_bana, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_banana",
      y = "count") 

# p value 0.04114 --> less than 0.05 so non-norma distribution
s1_brmp <- breath_wkly %>%
filter (supplement_consumed ==  "BRMPS") 
shapiro.test(s1_brmp$h2_mean)
ggplot(s1_brmp, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_brmps",
      y = "count") 
# p value= 2.823e-12; less than 0.05 so non-norma distribution

s1_brmp_acc <- breath_wkly %>%
filter (supplement_consumed ==  "BRMPS+Accessible") 
shapiro.test(s1_brmp_acc$h2_mean)
ggplot(s1_brmp_acc, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_brmps+Accesssible",
      y = "count") 
# p value = 0.05314

s1_himai <- breath_wkly %>%
filter (supplement_consumed ==  "HiMaize") 
shapiro.test(s1_himai$h2_mean)
ggplot(s1_himai, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_HiMaize",
      y = "count") 
# p value = 9.956e-06 less than 0.05 so non-norma distribution

s1_himai_brmps <- breath_wkly %>%
filter (supplement_consumed ==  "HiMaize+BRMPS") 
shapiro.test(s1_himai_brmps$h2_mean)
ggplot(s1_himai_brmps, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_HiMaize+BRMPS",
      y = "count") 
# p value = 0.001864 less than 0.05 so non-norma distribution

s1_inulin <- breath_wkly %>%
filter (supplement_consumed ==  "Inulin") 
shapiro.test(s1_inulin$h2_mean)
ggplot(s1_inulin, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_Inulin",
      y = "count")  
# p value = 0.0002929 less than 0.05 so non-norma distribution

s1_loodat <- breath_wkly %>%
filter (supplement_consumed ==  "LOODAT") 
shapiro.test(s1_loodat$h2_mean)
ggplot(s1_loodat, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_LOODAT",
      y = "count") 
# p value = 7.089e-05 less than 0.05 so non-norma distribution

s1_psyllium <- breath_wkly %>%
filter (supplement_consumed ==  "Psyllium") 
shapiro.test(s1_psyllium$h2_mean)
ggplot(s1_psyllium, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_psyllium",
      y = "count") 
# p value = 0.4093

s1_psyllium_brmp <- breath_wkly %>%
filter (supplement_consumed ==  "Psyllium+BRMPS") 
shapiro.test(s1_psyllium_brmp$h2_mean)
ggplot(s1_psyllium_brmp, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_LOODAT",
      y = "count") 
# p value = 0.472 

s1_transitionhi <- breath_wkly %>%
filter (supplement_consumed ==  "transition_HiMaize") 
shapiro.test(s1_transitionhi$h2_mean)
ggplot(s1_transitionhi, aes(x = h2_mean )) + geom_histogram()+
labs(x = "h2_mean_transition_HiMaize",
      y = "count") 
# p value = 7.102e-07 less than 0.05 so non-norma distribution

# check variances
bartlett.test(h2_mean ~ supplement_consumed, data = breath_wkly)

#we see some sample size below 30, and also the p value for normality , some supplements are normal distrbuted whereas some aren't who have less than 0.05 value, the vairence p value is 6.83 e-08 which means it is unequal
# so this is a partially normal and non-normal yet uneual variance 
```
...
```{r}
# conclusion 
# p value is 0.3, which fails to the null hypothesis so that the hydrogen breath levels don't differ between participants consuming fiber supplements
...
tax_ratio <- inner_join(Fr, Br) %>%
  ...
  tax_ratio_1 <- inner_join(tax_ratio, scfa_ratio) #join with scfa_ratio data frame
```
...
save_plot(filename = "Lab6/figures/correlation_butryate&firmicutes.pdf", 
          plot=plot_b_f,
           nrow = 3, ncol = 6, 
...
#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there could some linear relationship about butryate concentration and Firmicutes abudance; and the adjusted R-squared value is 0.003863, which means the dots are very losely fit into the straignt line (the correlation is not very monotonic).
```

Do you draw the same conclusions if you use the butyrate ratio and Firmicutes to Bacteroidetes ratio (f_b_ratio)?
```{r}
# plot
plot_b_f_ratio<- tax_ratio_1 %>%
  ...
  geom_smooth(method = "lm", #used to create a linear best fit line
              se = FALSE) + #hides confidence interval around line 
  xlab("butryate ratio") + 
  ylab("butyrate ratio and Firmicutes to Bacteroidetes ratio") 

save_plot(filename = "Lab6/figures/correlation_butyrate ratio and Firmicutes to Bacteroidetes ratio.pdf", 
          plot=plot_b_f_ratio,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
...
# correlation test 
m63b <- tax_ratio_1 %>%
  lm(butyrate_ratio ~ f_b_ratio, data = .) #test relationship
summary(m63b) #view results 

#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between butryate ratio and firmicutes to bacteroidetes ratio; and the adjusted R-squared value is 0.02854 , which means the dots are very losely fit into the straignt line (the correlation is not very monotonic).
#yes it's the same conclusion as before
```
...
save_plot(filename = "Lab6/figures/correlation_propriate_bacteroidetes.pdf", 
          plot=plot_pb,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
# correlation test 
...
#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between the propionate concentration and Bacteroidetes abundance; and the adjusted R-squared value is 0.04197, less than 0.8 , which means the dots are very losely fit into the straignt line (the correlation is not very monotonic).
```

Do you draw the same conclusions if you use the propionate ratio and Firmicutes to Bacteroidetes ratio?
```{r}
# plot
plot_p_f_ratio<- tax_ratio_1 %>%
...
save_plot(filename = "Lab6/figures/correlation of propionate ratio & Firmicutes to Bacteroidetes ratio.pdf", 
          plot= plot_p_f_ratio,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
...
# correlation test 
m63z <- tax_ratio_1 %>%
  lm(propionate_ratio ~ f_b_ratio, data = .) #test relationship
summary(m63z) #view results 
```
...
```{r}
# import 
pH_BSS_wkly <- read_delim("~/Documents/UMich_Bio201_F19/Lab4/curated_data/pH_BSS_wkly.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA"))
# join 
tax_scfa_pH <- inner_join(tax_ratio_1, pH_BSS_wkly) %>%
  na.omit(semester)
```
...
plot_pb<- tax_scfa_pH %>%
 ggplot(aes(x = totalscfa_mean,
  ...
  geom_smooth(method = "lm", #used to create a linear best fit line
              se = FALSE) + #hides confidence interval around line 
  xlab("totalscfa mean (mmol/kg)") + 
  ylab("mean acetate (mmol/kg)") 

save_plot(filename = "Lab6/figures/correlation_totalscfa and acetate.pdf", 
          plot=plot_pb,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
# correlation test 
m63h <- tax_scfa_pH %>%
  lm(totalscfa_mean ~ acetate_mean, data = .) #test relationship
summary(m63h) #view results 

#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between the propionate concentration and Bacteroidetes abundance; and the adjusted R-squared value is 0.87, bigger than 0.8 , which means the dots strongly fit into the straignt line (the correlation is very monotonic).
```
...
save_plot(filename = "Lab6/figures/correlation_totalscfa and acetate ratio.pdf", 
          plot=plot_pb,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
# correlation test 
m63h <- tax_scfa_pH %>%
  lm(totalscfa_mean ~ acetate_ratio, data = .) #test relationship
summary(m63h) #view results 

#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between the propionate concentration and Bacteroidetes abundance; and the adjusted R-squared value is 0.0188, smaller than 0.8 , which means the dots loosely fit into the straignt line (the correlation is not very monotonic).
```
...
save_plot(filename = "Lab6/figures/correlation_totalscfa and ph.pdf",
          plot=plot_pb,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
# correlation test 
m63h <- tax_scfa_pH %>%
  lm(totalscfa_mean ~ wkly_avg_ph, data = .) #test relationship
summary(m63h) #view results 

#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between the propionate concentration and Bacteroidetes abundance; and the adjusted R-squared value is 0.09942, smaller than 0.8 , which means the dots loosely fit into the straignt line (the correlation is not very monotonic).
```
...
save_plot(filename = "Lab6/figures/correlation_butryate and ph.pdf",
          plot=plot_pb,
           nrow = 3, ncol = 6, 
          base_aspect_ratio = 1.1)
# correlation test 
m63h <- tax_scfa_pH %>%
  lm(butyrate_mean ~ wkly_avg_ph, data = .) #test relationship
summary(m63h) #view results 

#the p value is  < 2.2e-16, which means we can use the linear model to interpreate the correlation that there is some linear reationship between the propionate concentration and Bacteroidetes abundance; and the adjusted R-squared value is 0.05209, smaller than 0.8 , which means the dots loosely fit into the straignt line (the correlation is not very monotonic).
```
